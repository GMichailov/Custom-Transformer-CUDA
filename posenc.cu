...
torch::Tensor yarn_encoding_unfused_cuda(torch::Tensor in, int64_t training_seq_len, int64_t extended_seq_len) {
    int batch_size = in.size(0);
    int seq_len = in.size(1);
    int n_heads = in.size(2);
    int head_dim = in.size(3);
    ...
    thrust::for_each(start, end,
        [=] __host__ __global__ (int idx) {
            ...
            if (dim % 2 == 0 && dim + 1 < head_dim) {
                ...
                float scaled_pos = (pos < training_seq_len) ? pos : training_seq_len + (pos - training_seq_len) * extended_seq_len;
                float theta = scaled_pos * powf(10000.0f, -2.0f * (dim/2) / (float)head_dim);
                float sin_t = sinf(theta);
                float cos_t = cosf(theta);
